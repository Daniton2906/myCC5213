...
Cell class
    Info:
        Store and manage the tuple (second, filename, distance) given from the knf computation
    Constructor:
        mytuple: str-tuple with (second, filename, distance)
...
class Cell:
    ...
    def __init__(self, mytuple):
        ...
        self.__value = float(mytuple[2])
        self.__file = mytuple[1]
        self.__second = float(mytuple[0])
    ...
    def get_tuple(self):
        return self.__second, self.__file, self.__value

    def __eq__(self, other):
        return other is not None and self.get_file() == other.get_file()

    def __str__(self):
        return "({}, {}, {})".format(self.__second, self.__file, self.__value)

    def compress_str(self):
        #return "({}, {})".format(Loader.get_original_name(self.__file), self.__frame)
        pass

    def is_next(self, other, n=0.5):
        return self.get_second() + n == other.get_second()


'''
KCell class
    Info:
        Store and manage a list of tuples (second, filename, distance) given from the knf computation
    Constructor:
        mytuple: list with tuples (second, filename, distance)
        k: number of Cell
